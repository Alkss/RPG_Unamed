...
class User
{
    ...
    public function createUse($name, $email, $password, $perfil, $active, $creationDate)
    {
        if ($this->checkIfExistsByEmail($email)) {
            ...
            $stringSQL = "INSERT INTO `tb_usuario` (nme_usuario, eml_usuario, lgn_usuario, pwd_usuario, cod_perfil, atv_usuario) VALUES (" . $this->db->scapeCont($name) . ", "
                . $this->db->scapeCont($email) . ", " . $md5Password . ", " . $this->db->scapeCont($perfil) . ", " . $this->db->scapeCont($active) . ", "
                . $this->db->scapeCont($creationDate) . ")";
    ...
    {
        $stringSQL = "SELECT * FROM `tb_usuario` WHERE email='" . $this->db->scapeCont($email) . "' ";
        $sql = $this->db->search($stringSQL);
        if (isset($sql)) {
            return true;
        }
